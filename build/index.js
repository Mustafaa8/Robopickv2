...
var bindIfParam = function(value, column4) {
...
var inArray = function(column4, values) {
...
var notInArray = function(column4, values) {
  if (Array.isArray(values)) {
    if (values.length === 0) {
...
var like = function(column4, value) {
  return sql`${column4} like ${value}`;
};
var notLike = function(column4, value) {
  return sql`${column4} not like ${value}`;
};
var ilike = function(column4, value) {
  return sql`${column4} ilike ${value}`;
};
var notIlike = function(column4, value) {
...
var extractTablesRelationalConfig = function(schema, configHelpers) {
  ...
  for (const [key, value] of Object.entries(schema)) {
    if (isTable(value)) {
      ...
      for (const column5 of Object.values(value[Table.Symbol.Columns])) {
        if (column5.primary) {
...
var aliasedTable = function(table8, tableAlias) {
  return new Proxy(table8, new TableAliasProxyHandler(tableAlias, false));
};
...
class SelectionProxyHandler {
  ...
  get(subquery3, prop) {
    if (prop === "_") {
      return {
        ...subquery3["_"],
        selectedFields: new Proxy(subquery3._.selectedFields, this)
      };
    }
    if (prop === ViewBaseConfig) {
      return {
    ...
    const columns = is(subquery3, Subquery) ? subquery3._.selectedFields : is(subquery3, View) ? subquery3[ViewBaseConfig].selectedFields : subquery3;
...
var sqliteTableBase = function(name, columns, extraConfig, schema, baseName = name) {
  ...
  table9[Table.Symbol.ExtraConfigColumns] = builtColumns;
  if (extraConfig) {
...
class SQLiteInsertBase extends QueryPromise {
  constructor(table13, values, session, dialect, withList) {
    super();
    this.session = session;
    this.dialect = dialect;
    this.config = { table: table13, values, withList };
  }
...
class ForeignKeyBuilder {
  ...
  build(table14) {
    return new ForeignKey(table14, this);
  }
}

class ForeignKey {
  constructor(table14, builder) {
    this.table = table14;
  ...
  getName() {
    ...
    const columnNames = columns.map((column8) => column8.name);
    const foreignColumnNames = foreignColumns.map((column8) => column8.name);
    const chunks = [
...
var uniqueKeyName2 = function(table15, columns) {
  return `${table15[SQLiteTable.Symbol.Name]}_${columns.join("_")}_unique`;
};
...
class SQLiteColumnBuilder extends ColumnBuilder {
  ...
  buildForeignKeys(column9, table15) {
    return this.foreignKeyConfigs.map(({ ref, actions }) => {
      return ((ref2, actions2) => {
        const builder = new ForeignKeyBuilder(() => {
          const foreignColumn = ref2();
          return { columns: [column9], foreignColumns: [foreignColumn] };
        });
...
class SQLiteColumn extends Column {
  constructor(table15, config) {
    if (!config.uniqueName) {
      config.uniqueName = uniqueKeyName2(table15, [config.name]);
    }
    super(table15, config);
    this.table = table15;
  }
...
class SQLiteIntegerBuilder extends SQLiteBaseIntegerBuilder {
  ...
  build(table15) {
    return new SQLiteInteger(table15, this.config);
  }
}
...
class SQLiteTimestampBuilder extends SQLiteBaseIntegerBuilder {
  ...
  build(table15) {
    return new SQLiteTimestamp(table15, this.config);
  }
}
...
class SQLiteBooleanBuilder extends SQLiteBaseIntegerBuilder {
  ...
  build(table15) {
    return new SQLiteBoolean(table15, this.config);
  }
}
...
var db3 = drizzle(sqlite);
var db_default = db3;

// db/schema.ts
var schema = sqliteTable("points", {
...
api.get("/first", async (req, res, next) => {
  try {
    const first = await db_default.select().from(schema_default).limit(1);
    let id = first[0]["id"];
    if (id == undefined) {
      next();
    }
    res.json(first);
    await db_default.delete(schema_default).where(eq(schema_default.id, id));
  } catch (err) {
    next(err);
  }
});
...
app.post("/task", async (req, res, next) => {
  ...
  try {
    const point = await db_default.insert(schema_default).values({
